...
const uint LED = 12;      // Pino para o LED (original, não utilizado na nova ação)

// Definições adicionais para o LED RGB
const uint LED_R = 12;  // Pino para o canal vermelho do LED RGB
const uint LED_G = 13;  // Pino para o canal verde do LED RGB
const uint LED_B = 14;  // Pino para o canal azul do LED RGB
...
const uint16_t LED_STEP = 100;           // Passo para controle de brilho do LED (não utilizado na nova ação)
...
uint16_t led_level = 100;     // Nível de brilho do LED (não será utilizado para o LED RGB)
...
void read_buttons()
{
  ...
  case ACTION_A:
    s = IDLE;
    // Ação do Botão A: Define o LED para cor vermelha usando LED RGB.
    pwm_set_gpio_level(LED_R, PERIOD); // Ativa o canal vermelho (brilho máximo)
    pwm_set_gpio_level(LED_G, 0);        // Desativa o canal verde
    pwm_set_gpio_level(LED_B, 0);        // Desativa o canal azul
    if (wrap_div_buzzer < MAX_WRAP_DIV_BUZZER)
      wrap_div_buzzer += MIN_WRAP_DIV_BUZZER;
    break;
  case ACTION_B:
    s = IDLE;
    // Ação do Botão B: Define o LED para cor verde usando LED RGB.
    pwm_set_gpio_level(LED_R, 0);        // Desativa o canal vermelho
    pwm_set_gpio_level(LED_G, PERIOD);   // Ativa o canal verde (brilho máximo)
    pwm_set_gpio_level(LED_B, 0);        // Desativa o canal azul
    if (wrap_div_buzzer > MIN_WRAP_DIV_BUZZER)
      wrap_div_buzzer -= MIN_WRAP_DIV_BUZZER;
    break;
